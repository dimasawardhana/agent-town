...
- [x] The extension is discovered from `.omp/extensions/` with no CLI flag, and from `~/.omp/agent/extensions/`
- [x] On session start the extension POSTs a `hello` frame carrying the session id, and the daemon logs the handshake
- [x] Driving omp to read a file produces a `FILE_READ` event with the real path and `result: "success"`
- [x] Driving omp to read a nonexistent file produces `result: "error"` with the failing path preserved
- [x] Running omp from an unwatched directory produces **zero** events, and the daemon logs a 403 rejection
- [x] The extension never blocks a tool: only observation hooks are subscribed
- [x] With the daemon stopped, omp completes normally and the agent is unaffected
- [x] A session whose daemon is unreachable when it starts flushes its buffered frames once the daemon returns, or at exit via `session_shutdown`
...
- [x] With `AI_TOWN_URL` unset, the extension is completely inert
- [x] The extension typechecks with no imports and no dependencies
